...
- **翻译设置**：`GET/PUT /settings/translation`、 `POST /settings/translation/test`（免费 Google 通道，目标语言可配置）。契约见 [`contracts/openapi.json`](contracts/openapi.json)。
...
- **设置**（`/settings`）：语雀式单页锚点导航，分区 **通用**（主题循环小方钮）、**翻译**。
...
产物：`src-tauri\target\release\bundle\nsis\Project Pilot_0.1.0_x64-setup.exe`（当前为 NSIS；MSI 需 WiX 且首次会下载工具链）。
...
- **刷新策略**：手动刷新（顶栏、侧栏「刷新全部」、再次点击当前频道）带 **`fresh=true`** 绕过 SQLite 缓存；**5 分钟内**切换频道等被动请求复用缓存。侧栏 **趋势**、**热门发布** 显示「上次刷新」；**主题探索**、**最受欢迎** 不显示。
