...
> 关联文档：[PROJECT_PILOT_v0.1_设计文档.md](./PROJECT_PILOT_v0.1_设计文档.md)、[PROJECT_PILOT_Implementation_Plan.md](./PROJECT_PILOT_Implementation_Plan.md)、[桌面安装包打包经验.md](./桌面安装包打包经验.md)、[AGENTS.md](../AGENTS.md)
...
- [x] 初始化 `src-tauri/`，`tauri dev` 能加载 `frontend`。
- [x] PyInstaller 打出 `project-pilot-api` sidecar；Tauri `externalBin` 声明。
- [x] Rust：单实例、启停 sidecar、`/health` 等待、退出清理。
...
**首次发版逐步操作**见专用文档：[PROJECT_PILOT_Release_Checklist.md](./PROJECT_PILOT_Release_Checklist.md)（含 `v0.1.0` 与 GitHub Releases 上传说明）。
...
*Phase 0 实施决策（2026-05-24）：*

| 项 | 决策 |
|----|------|
| 生产 `/api` | **方案 A** — sidecar 提供 `STATIC_DIR` + `/api`；WebView 打开 `http://127.0.0.1:38472/` |
| 平台 | **Windows x64** |
| 开发 | `npm run tauri:dev` + 手动 `uvicorn`；debug 不启 sidecar |
| 端口 | 固定 **38472** |
| 构建 | [`scripts/build-sidecar.ps1`](../scripts/build-sidecar.ps1) → [`scripts/build-desktop.ps1`](../scripts/build-desktop.ps1) |
