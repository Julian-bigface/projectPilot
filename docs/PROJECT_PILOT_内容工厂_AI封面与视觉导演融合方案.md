...
| 风格库 → **AI 调整（整套 refine）** | ✅ | 详情头 `POST .../cover-styles/refine`（2026-06-25） |
| 风格库 → **视觉解构可编辑** | ✅ | `design_analysis` PATCH（2026-06-25） |
...
- 不实现跨项目库风格共享 / 市场导入。  
- **风格版本历史（Phase 2.x）**：AI 调整应用成功后写入 `content_factory_cover_style_revisions` 完整快照（含当时示例图副本）；详情页 Version Rail 可回退预览并写回编辑区，持久化仍走「保存修改 / Fork 并保存」。每风格最多 20 条修订。  
...
| **Phase 2.5+** | 参考图 vision 解析 + **整套 AI refine** + 视觉解构编辑 | ✅ 2026-06-22～06-25 |
| **Step 4 导出发布** | 小红书手机预览、复制发布包、Stepper、`published` 状态 | ✅ 2026-06-26（[`CHANGELOG_2026-06-26`](../changelogs/CHANGELOG_2026-06-26.md)） |
| **Phase 3** | 轮播图 Tab（6–8 页）+ `publish_images[]` 多图 | ⏳ UI Tab 仍 disabled |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.9 | 2026-06-27 | Step 4 导出发布、Phase 2.5+ refine / 解构编辑；状态同步待办清单 |
